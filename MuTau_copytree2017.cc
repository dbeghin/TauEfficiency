...
void IIHEAnalysis::Loop(string phase, string type_of_data, string out_name, string whocares, TH1F* h1, TH1F* h2) {
   ...
   for (Long64_t jEntry = 0; jEntry < nEntries; ++jEntry) {
      ...
      if (trig_HLT_IsoMu27_accept /*|| trig_HLT_IsoTkMu24_accept*/ || trig_HLT_Mu50_accept /*|| trig_HLT_TkMu50_accept*/) PassMuonTrigger = true;
	...
	for (unsigned int iMu2 = 0; iMu2 < mu_gt_pt->size(); ++iMu2) {
	  ...
	  if (mu_pfIsoDbCorrected04->at(iMu2) > 0.3) continue;
	  ...
	  /*if (!data) {
	    if (mc_weight == 0) {
	      ...
	      event_weight = mc_weight/fabs(mc_weight)*GetReweight_mumu(mc_trueNumInteractions, mu_gt_pt->at(iMu), mu_gt_eta->at(iMu), mu_gt_pt->at(iMu2), mu_gt_eta->at(iMu2));
	    }
	    }*/
	  if (tauormu) break;
	}//second muon loop
	...
	for (unsigned int iTau = 0; iTau < tau_pt->size(); ++iTau) {
	  ...
	  /*if (!data) {
	    if (mc_weight == 0) {
	      ...
	      event_weight = mc_weight/fabs(mc_weight)*GetPUTriggerMuonIDMuonIsoReweight(mc_trueNumInteractions, mu_gt_pt->at(iMu), mu_gt_eta->at(iMu));
	    }
	  }*/
