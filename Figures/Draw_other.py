...
#file=ROOT.TFile("Wjets_pre_estimation.root","r")
file=ROOT.TFile("QCD_for_plotting.root","r")
#file=ROOT.TFile("Wjets_post_estimation.root","r")
#file=ROOT.TFile("antimu_control.root","r")
    ...
    for j in range (0,ntauID):
        for i in range (0,ncat):
            if k < tauID_cutoff or j == 0:
                ...
                ph_tauID_in = photogenic_tauIDs[tauIDs[j]]
        ...
        #DYS.Scale(0.84) #FIXME
            ...
            legende.AddEntry(DYB,"All DY","f")
            legende.AddEntry(TTB,"t#bar{t}+jets","f")
            legende.AddEntry(W,"Electroweak","f")
            if i<4:
                legende.AddEntry(QCD,"QCD multijet (MC)","f")
