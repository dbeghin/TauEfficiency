...
def make_legend2():
    output = ROOT.TLegend(0.45, 0.4, 0.72, 0.82, "", "brNDC")
    output.SetLineWidth(0)
    output.SetLineStyle(0)
    output.SetFillStyle(0)
    output.SetBorderSize(0)
...
#file=ROOT.TFile("allhistos.root","r")
...
tauIDs=[
"noTauID",
#"cutbased_loose","cutbased_medium","cutbased_tight","MVA_veryloose","MVA_loose",
"MVA_medium",
"MVA_tight",
#"MVA_verytight","MVA_veryverytight"
]
photogenic_tauIDs=[
"no tau ID",
#"Cut-based loose","Cut-based medium","Cut-based tight","MVA very loose","MVA loose",
"MVA medium",
"MVA tight",
#"MVA very tight","MVA very very tight"
]
ntauID=len(tauIDs)
categories=["pass","fail"]#"pass_postfit","fail_postfit","zmm_prefit","zmm_postfit"]
ncat=2 #6

for k in range (0,nvar):
    for j in range (0,ntauID):
        for i in range (0,ncat):
            if k < tauID_cutoff or j == 0:
                if j>0 or i>0:
                    continue
                var_in = var[k]
                if k >= tauID_cutoff: var_in = var[k]+"_"+tauIDs[j]
                ph_tauID_in = "No tau ID"
                categ_in = ""
            else:
                var_in = var[k]+"_"+tauIDs[j]+"_"+categories[i]
                ph_tauID_in = photogenic_tauIDs[j]
                categ_in = categories[i]
            print var_in
            Data=file.Get("data_"+var_in)
            QCD=file.Get("QCD_"+var_in)
            W=file.Get("WJets_"+var_in)
            TTS=file.Get("TTS_"+var_in)
            TTB=file.Get("TTB_"+var_in)
            ...
            if k == nvar-1:
                ...
                nevents_file.write(photogenic_tauIDs[j] + " " + categories[i] + "\n\n")
                nevents_file.write("processus | nevents \n")
                nevents_file.write("DY signal   " + str(DYS.Integral()) + "\n")
                nevents_file.write("DY bg       " + str(DYB.Integral()) + "\n")
                nevents_file.write("QCD         " + str(QCD.Integral()) + "\n")
                nevents_file.write("WJets       " + str(W.Integral()) + "\n")
                nevents_file.write("VV          " + str(VV.Integral()) + "\n")
                nevents_file.write("TT          " + str(TTS.Integral()) + "\n")
            ...
            TTS.SetFillColor(ROOT.TColor.GetColor("#9999cc"))
        ...
        #DYS.Scale(0.84) #FIXME
            ...
            legende.AddEntry(TTS,"t#bar{t}+jets (real #tau)","f")
