...
//Main analysis code
void IIHEAnalysis::Loop(string phase, string type_of_data, string in_name, string out_name, string mc_nick, Float_t nEvents) {
   if (fChain == 0) return;
   ...
   vector<TH2F*> htau[HPS_WP.size()][passfail.size()][dms.size()];
   for (unsigned int i1 = 0; i1<HPS_WP.size(); ++i1) {
	 ...
	 for (unsigned int k = 0; k<eta.size(); ++k) {
	   htau[i1][i2][j].push_back( new TH2F("taupt_ratio_"+HPS_WP[i1]+"_"+passfail[i2]+"_"+dms[j]+"_"+eta[k], "taupt_ratio_"+HPS_WP[i1]+"_"+passfail[i2]+"_"+dms[j]+"_"+eta[k], 1000, 0, 1000, 1000, 0, 10) ); 
	 }
       }
     }
   }
	...
	for (unsigned int iMu2 = 0; iMu2 < iMu1; ++iMu2) {
	  ...
	  for (unsigned int iTau = 0; iTau < tau_pt->size(); ++iTau) {
		...
		if (tauIDvalues[iID] > 0.5) {
		  htau[iID][0][j_dm][k_eta]->Fill(tau_pt->at(iTau), ratio, final_weight);
		}
		else {
		  htau[iID][1][j_dm][k_eta]->Fill(tau_pt->at(iTau), ratio, final_weight);
		}
	      }
	      else {
