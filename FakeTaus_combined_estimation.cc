...
int main(int argc, char** argv) {
  ...
  vars.push_back("ev_Mvis_TES_DM0_down");
  vars.push_back("ev_Mvis_TES_DM0_up");
  vars.push_back("tau_pt_TES_DM0_down");
  vars.push_back("tau_pt_TES_DM0_up");
  vars.push_back("ev_Mvis_TES_DM1_down");
  vars.push_back("ev_Mvis_TES_DM1_up");
  vars.push_back("tau_pt_TES_DM1_down");
  vars.push_back("tau_pt_TES_DM1_up");
  vars.push_back("ev_Mvis_TES_DM10_down");
	...
	for (unsigned int l=0; l<eta.size(); ++l) {
	  for (unsigned int m=0; m<HPS_WP.size(); ++m) {
	    for (unsigned int p=0; p<ptrange.size(); ++p) {
	      cout << HPS_WP[m]+"/"+names[i]+vars[j]+"_"+dms[k]+"_"+eta[l]+"_"+ptrange[p]+"_"+HPS_WP[m]+"_fail" << endl;
